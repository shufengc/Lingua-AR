...
namespace OpenCVForUnityExample.DnnModel
{
    ...
    public class YOLOXObjectDetector
    {
        ...
        public List<string> get_classNames()
        {
            return classNames;
        }
        ...
        void Set_Info_Active(string originalText, GameObject O_TranslationInfo, AudioSource O_AudioSource)
        {
            Debug.Log("Clicked");
            ...
            if (O_TranslationInfo == null)
            {
                Debug.Log("Info no found");
            }

            O_AudioSource.clip = null;

            O_TranslationInfo.SetActive(true);
            ...
            if (O_translatedText == null)
            {
                Debug.Log("T Text no found");
            }
            if (O_originalText == null)
            {
                Debug.Log("O Text no found");
            }
            if (O_translatedLang == null)
            {
                Debug.Log("T Lang no found");
            }

            O_translatedText.GetComponent<Text>().text = "";
            O_originalText.GetComponent<Text>().text = originalText;
            O_translatedLang.GetComponent<Text>().text = display_targeted_lang;

            GameObject Quad = GameObject.Find("Quad");
            Quad.GetComponent<ObjectDetectionYOLOXExample>().Translate();
        }

        public List<GameObject> buttons = new List<GameObject>();

        public virtual void visualize(ref Mat image, Mat results, bool print_results = false, bool isRGB = false, bool paused = false, GameObject O_TranslationInfo = null, AudioSource O_AudioSource = null)
        {
            if (image.IsDisposed)
                return;
            ...
            GameObject ObjButton = (GameObject)Resources.Load("Prefabs/ObjButton");
            ...
            for (int i = results.rows() - 1; i >= 0; --i)
            {
                ...
                {
                    // Generate the button
                    double horizontal_scale = right - left;
                    double vertical_scale =  bottom - top;
                    Debug.Log("Rect Location: left="+left+",top="+top+",right="+right+",bottom="+bottom+",hor scale:"+horizontal_scale+",ver scale:"+vertical_scale);
                    GameObject o_button = GameObject.Instantiate(ObjButton, new Vector3(2 * left, h - 2 * bottom, 0), Quaternion.identity, Canvas.transform);
                    o_button.transform.localScale = new Vector3(2 * (float)horizontal_scale, 2 * (float)vertical_scale, 0);
                    Debug.Log(o_button.transform.position);
                    string originalLabel = classNames[classId];
                    o_button.GetComponent<Button>().onClick.AddListener(delegate { Set_Info_Active(originalLabel, O_TranslationInfo, O_AudioSource); });
